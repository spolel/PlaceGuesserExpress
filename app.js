...
app.get("/get_random_place", async (req, res) => {
  ...
  for (let i = geocodeResult.length - 1; i >= 0; i--) {
    ...
    if (
      geocodeResult[i].formatted_address
        .split(",")[0]
        .toLowerCase()
        .includes(place[0].asciiname.replace(/[^a-zA-Z ]/g, " ").toLowerCase())
    ) {
      containsNameIndex = i;
      break;
    }
  ...
  details.name = geocodeResult[placeIndex].formatted_address
    .split(",")[0]
    .replace(/[0-9]/g, "");
...
app.get("/get_stats", async (req, res) => {
  ...
  try {
    ...
    data.forEach((x) => {
      distribution.push(x.score);

      if (x.score > highscore) {
        highscore = x.score;
      }
    });
  ...
  const stats = { played, highscore, distribution, rank };
...
function generateScore(distance, zoneMode, gameMode, countryCode) {
  ...
  if (gameMode == "country") {
    const diagonal = getDistanceFromLatLonInKm(
      bounds[countryCode][1][0],
      bounds[countryCode][1][1],
      bounds[countryCode][1][2],
      bounds[countryCode][1][3]
    );
  ...
  if (distance > maxDistance) {
    ...
    return Math.floor(
      1000 *
        (1 - (distance - maxPointsCutoff) / (maxDistance - maxPointsCutoff)) **
          2
    );
  }
}
